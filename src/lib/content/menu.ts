...
export type MenuEntry = {
    ...
    slug: string;
    body: string;
    collection: CollectionEntry<"menu">["collection"];
    data: MenuData;
}
...
export async function get_root_menu(all_menu_items?: AnyMenuEntry[]): Promise<AnyMenuEntry[]> {
...
export function synthesize_menu_entry(
    ...
    const id_parts = id.replace(/\/$/, '').replace(/\.\w+$/, '').split('/');
    const slug_parts = parent ? parent.slug.replace(/\/index$/, '').split('/').filter(part => part.length > 0) : [];
...
export async function get_breadcrumbs(url: string, title: string, body: string, all_menu_items?: AnyMenuEntry[]): Promise<AnyMenuEntry[]> {
    if (typeof all_menu_items == 'undefined') {
        all_menu_items = await get_all_menu_items();
    }
    ...
    if (typeof settings !== 'undefined') {
        ...
        site_crumb = synthesize_menu_entry(
            ...
            settings.data.url ? new URL(settings.data.url).pathname : `${url_obj.protocol}${url_obj.host}/`,
            settings.data.breadcrumb_description || settings.body || settings.data.breadcrumb_text ? settings.data.title : "",
            -1
        )
    } else {
    ...
    let current_menu: AnyMenuEntry | undefined = await get_menu_for_url(url, all_menu_items)
